...
fn main() {
    ...
    println!(
        "Matrix: [{intermediate}, {hidden}] = {:.1}MB f32 → {:.1}MB Q4_0",
    ...
    // 1. BLAS f32 gemv
    {
        ...
        println!(
            "  BLAS f32 gemv:     {ms:>6.2}ms  ({gbps:>5.1} GB/s on {:.1}MB)",
            (intermediate * hidden * 4) as f64 / 1e6
        );
    }
    ...
    // 2. C Q4 kernel (via CPU backend)
    {
        ...
        println!(
            "  CPU Q4 kernel:     {ms:>6.2}ms  ({gbps:>5.1} GB/s on {:.1}MB)",
            q4_data.len() as f64 / 1e6
        );
    }
    ...
    if default.has_q4() && default.name() != cpu.name() {
        ...
        let gbps = q4_data.len() as f64 / ms / 1e6;
        println!(
            "  {} Q4:  {ms:>6.2}ms  ({gbps:>5.1} GB/s on {:.1}MB)",
            default.name(),
            q4_data.len() as f64 / 1e6
        );
    }

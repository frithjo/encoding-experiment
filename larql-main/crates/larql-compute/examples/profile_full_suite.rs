...
fn main() {
    ...
    println!("\n--- 2. f32 matmul (a @ b, FFN down) ---\n");
    {
        ...
        bench.run("CPU  [6,10240] @ [10240,2560]", bytes, || {
            let _ = cpu.matmul(act.view(), down.view());
        });
        if default.name() != cpu.name() {
            bench.run(
                &format!("{}  [6,10240] @ [10240,2560]", default.name()),
                bytes,
                || {
    ...
    println!("\n--- 5. Q4 batched gate+up (6 positions, 1 submission) ---\n");
    {
        ...
        if default.has_q4() {
            let result =
                default.q4_matvec_pair_batch(&gate_q4, &up_q4, &x_matrix, 6, inter, hidden);
            if let Some((gate_scores, up_scores)) = result {
                println!(
                    "    Batch returned: {} gate × {} up scores per position",
                    gate_scores[0].len(),
                    up_scores[0].len()
                );
    ...
    println!("\n--- 6. Multi-layer simulation (21 layers, f32 FFN) ---\n");
    {
        ...
        if default.name() != cpu.name() {
            bench.run(
                &format!("{} 21 layers × 3 matmuls", default.name()),
                bytes,
                || {
                    let mut h = x.clone();
                    for (gate, up, down) in &layers {
    ...
    #[cfg(feature = "metal")]
    {
        if let Some(ref metal) = larql_compute::metal::MetalBackend::new_default() {
    ...
    println!("\n--- 10. Correctness (CPU vs Default) ---\n");
    {
        ...
        let diff: f32 = cpu_result
            .iter()
            .zip(default_result.iter())
            .map(|(x, y)| (x - y).abs())
            .fold(0.0f32, f32::max);
        println!(
            "  f32 matmul_transb max diff: {diff:.2e} {}",
            if diff < 1e-4 { "✓" } else { "✗" }
        );
        ...
        if cpu.has_q4() && default.has_q4() {
            ...
            let cpu_q4 = cpu
                .q4_matvec(&q4_data, &q8_x, &q8_scales, inter, hidden)
                .unwrap();
            let def_q4 = default
                .q4_matvec(&q4_data, &q8_x, &q8_scales, inter, hidden)
                .unwrap();

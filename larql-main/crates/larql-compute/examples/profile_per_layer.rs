    ...
    #[cfg(feature = "metal")]
    {
        ...
        for &num_layers in &[1usize, 21] {
            ...
            for l in 0..num_layers {
                let wq = quantize_q4_k(&pad(&(0..q_dim * hidden)
                    .map(|i| ((i + l * 1000) as f32 * 0.0001).cos())
                    .collect::<Vec<_>>()));
                let wk = quantize_q4_k(&pad(&(0..kv_dim * hidden)
                    .map(|i| ((i + l * 2000) as f32 * 0.0002).sin())
                    .collect::<Vec<_>>()));
                let wv = quantize_q4_k(&pad(&(0..kv_dim * hidden)
                    .map(|i| ((i + l * 3000) as f32 * 0.0003).cos())
                    .collect::<Vec<_>>()));
                let wo = quantize_q4_k(&pad(&(0..hidden * q_dim)
            ...
            let layers: Vec<larql_compute::FullPipelineLayer> = layers_data
                .iter()
                .map(
                    |(wq, wk, wv, wo, g, u, d, norm)| larql_compute::FullPipelineLayer {
            ...
            for _ in 0..3 {
                let _ = metal.full_pipeline_q4(
                    &layers, &x, hidden, inter, q_dim, kv_dim, 1, num_q, num_kv, hd, 10000.0,
                    false, 0.0,
                );
            }
            ...
            let data_mb = layers_data
                .iter()
                .map(|(q, k, v, o, g, u, d, _)| {
                    q.len() + k.len() + v.len() + o.len() + g.len() + u.len() + d.len()
                })
                .sum::<usize>() as f64
                / 1e6
                / num_layers as f64;

...
fn main() {
    ...
    #[cfg(feature = "metal")]
    {
        ...
        let metal = larql_compute::metal::MetalBackend::new_default().expect("Metal required");
        ...
        let qkv_ms = bench!("Q4_K QKV fused", {
            ...
            for _ in 0..layers {
                ...
                enc.dispatch_thread_groups(
                    ...
                    metal::MTLSize::new(qkv_sh::THREADS_PER_TG, 1, 1),
                );
                enc.end_encoding();
            }
            cmd.commit();
        ...
        let kv_ms = bench!("KV cache append+attend", {
            ...
            for _l in 0..layers {
                ...
                enc.set_buffer(1, Some(&buf_norm), 0);
        ...
        let o_ms = bench!("Q4_K O projection", {
            ...
            for _ in 0..layers {
                ...
                enc.set_buffer(0, Some(&buf_wo), 0);
                enc.set_buffer(1, Some(&buf_wo), 0);
                enc.set_buffer(2, Some(&buf_wo), 0);
                enc.set_buffer(3, Some(&buf_x), 0);
                enc.set_buffer(4, Some(&oo), 0);
                enc.set_buffer(5, Some(&oo), 0);
                enc.set_buffer(6, Some(&oo), 0);
                let nr = hidden as u32;
                let z = 0u32;
                let h = q_dim as u32;
        ...
        let ffn_ms = bench!("Q4 FFN (gate+up+geglu+down)", {
            ...
            for _ in 0..layers {
                ...
                enc.dispatch_thread_groups(
                    ...
                    metal::MTLSize::new(q4mv::THREADS_PER_TG, 1, 1),
                );
                // up
                enc.set_buffer(0, Some(&buf_up), 0);
                enc.set_buffer(3, Some(&uo), 0);
                enc.dispatch_thread_groups(
                    metal::MTLSize::new(n_tgs, 1, 1),
                    metal::MTLSize::new(q4mv::THREADS_PER_TG, 1, 1),
                );
                ...
                enc.set_buffer(0, Some(&buf_down), 0);
                enc.set_buffer(1, Some(&ao), 0);
                enc.set_buffer(2, Some(&do_), 0);
                enc.set_bytes(3, 4, &hidden_val as *const u32 as *const c_void);
                enc.set_bytes(4, 4, &inter_val as *const u32 as *const c_void);
                enc.dispatch_threads(
                    metal::MTLSize::new(hidden as u64, 1, 1),
        ...
        let overhead_ms = bench!("empty encoder overhead", {
            let cmd = metal.queue().new_command_buffer();
            for _ in 0..layers * 7 {
                // 7 encoders per layer in decode
                let enc = cmd.new_compute_command_encoder();
                enc.end_encoding();
            }
            cmd.commit();
        ...
        println!(
            "  Encoder overhead:   {overhead_ms:.1}ms ({:.0} empty encoders)",
            layers as f64 * 7.0
        );

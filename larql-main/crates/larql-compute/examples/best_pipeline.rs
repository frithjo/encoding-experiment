...
fn main() {
    ...
    #[cfg(feature = "metal")]
    {
        ...
        let metal = MetalBackend::new_default().expect("Metal required");
        ...
        for l in 0..num_layers {
            let wq: Vec<f32> = (0..q_dim * hidden)
                .map(|i| ((i + l * 1000) as f32 * 0.0001).cos())
                .collect();
            let wk: Vec<f32> = (0..kv_dim * hidden)
                .map(|i| ((i + l * 2000) as f32 * 0.0002).sin())
                .collect();
            let wv: Vec<f32> = (0..kv_dim * hidden)
            ...
            for r in 0..inter {
                for c in 0..hidden {
                    dt[c * inter + r] = ((r * hidden + c + l * 7000) as f32 * 0.0003).cos();
                }
            }
            layers_data.push(LayerData {
                wq_q4: quantize_q4_0(&wq),
                wk_q4: quantize_q4_0(&wk),
                wv_q4: quantize_q4_0(&wv),
                wo_q4: quantize_q4_0(&wo),
                gate_q4: quantize_q4_0(&g),
                up_q4: quantize_q4_0(&u),
                down_t_q4: quantize_q4_0(&dt),
            });
        }

        let layers: Vec<LayerWeights> = layers_data
            .iter()
            .map(|ld| LayerWeights {
                wq_q4: &ld.wq_q4,
                wk_q4: &ld.wk_q4,
        ...
        println!(
            "  Attention overhead in pipeline:          {:.1}ms",
            full_ms - ffn_ms
        );
        println!();

...
fn main() {
    ...
    #[cfg(feature = "metal")]
    {
        use larql_compute::cpu::ops::q4_common::{
            quantize_q4_k, quantize_q4_k_gguf, quantize_to_q8,
        };
        use larql_compute::ComputeBackend;
        use std::time::Instant;

        let metal_raw = larql_compute::metal::MetalBackend::new_default().expect("Metal required");
            ...
            (0..count)
                .map(|l| {
                    ...
                    Layer {
                        ...
                        wv8s: q8vs,
                        wo8s: q8os,
                        g: quantize_q4_k_gguf(&pad(&(0..inter * hidden)
                            .map(|i| ((i + l * 5000) as f32 * 0.0001).cos())
                            .collect::<Vec<_>>())),
                        u: quantize_q4_k_gguf(&pad(&(0..inter * hidden)
                            .map(|i| ((i + l * 6000) as f32 * 0.0002).sin())
                            .collect::<Vec<_>>())),
                        d: quantize_q4_k_gguf(&pad(&(0..hidden * inter)

...
impl MetalBackend {
    ...
    pub fn new() -> Option<Self> {
        ...
        let causal_attn_pipeline = device
            .new_compute_pipeline_state_with_function(&causal_attn_fn)
            .ok()?;

        let q4 = Q4Pipelines {
            matvec: device
                .new_compute_pipeline_state_with_function(&q4_matvec_fn)
                .ok()?,
            vecmat: device
                .new_compute_pipeline_state_with_function(&q4_vecmat_fn)
                .ok()?,
            f32_matvec: device
                .new_compute_pipeline_state_with_function(&q4_f32_matvec_fn)
                .ok()?,
        };

        let bufs = BufferCache::new(&device);

        let geglu_pipeline = device
            .new_compute_pipeline_state_with_function(&geglu_fn)
            .ok()?;
        let geglu_gelu_tanh_fn = library.get_function("geglu_gelu_tanh", None).ok()?;
        let geglu_gelu_tanh_pipeline = device
            .new_compute_pipeline_state_with_function(&geglu_gelu_tanh_fn)
            .ok()?;
        let q8_quant_pipeline = device
            .new_compute_pipeline_state_with_function(&q8_quant_fn)
            .ok()?;

        // Q8 matvec for attention projections
        let q8_matvec_fn = library.get_function("q8_matvec", None).ok()?;
        let q8_matvec_pipeline = device
            .new_compute_pipeline_state_with_function(&q8_matvec_fn)
            .ok()?;
        ...
        let rms_norm_pipeline = device
            .new_compute_pipeline_state_with_function(&rms_norm_fn)
            .ok()?;
        let residual_add_pipeline = device
            .new_compute_pipeline_state_with_function(&residual_add_fn)
            .ok()?;
        ...
        let q4k_matvec_pipeline = device
            .new_compute_pipeline_state_with_function(&q4k_fn)
            .ok()?;
        let q4k_ffn_gate_up_pipeline = device
            .new_compute_pipeline_state_with_function(&q4k_ffn_gate_up_fn)
            .ok()?;
        let q4kf_ffn_gate_up_fn = library.get_function("q4kf_ffn_gate_up", None).ok()?;
        let q4kf_ffn_gate_up_pipeline = device
            .new_compute_pipeline_state_with_function(&q4kf_ffn_gate_up_fn)
            .ok()?;
        let q4k_geglu_silu_down_fn = library.get_function("q4k_geglu_silu_down", None).ok()?;
        let q4k_geglu_silu_down_pipeline = device
            .new_compute_pipeline_state_with_function(&q4k_geglu_silu_down_fn)
            .ok()?;
        let q4k_geglu_gelu_tanh_down_fn = library
            .get_function("q4k_geglu_gelu_tanh_down", None)
            .ok()?;
        let q4k_geglu_gelu_tanh_down_pipeline = device
            .new_compute_pipeline_state_with_function(&q4k_geglu_gelu_tanh_down_fn)
            .ok()?;
        let q6k_matvec_pipeline = device
            .new_compute_pipeline_state_with_function(&q6k_fn)
            .ok()?;

        // Fused Q8 QKV projection (all 3 in one dispatch)
        let q8_qkv_fn = library.get_function("q8_qkv_proj", None).ok()?;
        let q8_qkv_proj_pipeline = device
            .new_compute_pipeline_state_with_function(&q8_qkv_fn)
            .ok()?;
        ...
        let rms_norm_q8_pipeline = device
            .new_compute_pipeline_state_with_function(&rms_norm_q8_fn)
            .ok()?;
        let residual_norm_pipeline = device
            .new_compute_pipeline_state_with_function(&residual_norm_fn)
            .ok()?;
        let residual_norm_q8_pipeline = device
            .new_compute_pipeline_state_with_function(&residual_norm_q8_fn)
            .ok()?;
        ...
        let rope_at_pos_pipeline = device
            .new_compute_pipeline_state_with_function(&rope_at_pos_fn)
            .ok()?;
        let rope_at_pos_batched_fn = library.get_function("rope_at_pos_batched", None).ok()?;
        let rope_at_pos_batched_pipeline = device
            .new_compute_pipeline_state_with_function(&rope_at_pos_batched_fn)
            .ok()?;

        // Fused Q4_K QKV projection (one dispatch for Q+K+V)
        let q4k_qkv_fn = library.get_function("q4k_qkv_proj", None).ok()?;
        let q4k_qkv_proj_pipeline = device
            .new_compute_pipeline_state_with_function(&q4k_qkv_fn)
            .ok()?;
        let q4k_proj_fn = library.get_function("q4k_proj", None).ok()?;
        let q4k_proj_pipeline = device
            .new_compute_pipeline_state_with_function(&q4k_proj_fn)
            .ok()?;

        // Q4_KF: pre-baked scales (faster inference)
        let q4kf_qkv_fn = library.get_function("q4kf_qkv_proj", None).ok()?;
        let q4kf_qkv_proj_pipeline = device
            .new_compute_pipeline_state_with_function(&q4kf_qkv_fn)
            .ok()?;
        let q4kf_proj_fn = library.get_function("q4kf_proj", None).ok()?;
        let q4kf_proj_pipeline = device
            .new_compute_pipeline_state_with_function(&q4kf_proj_fn)
            .ok()?;

        // Fused attention (RoPE + GQA + softcap)
        let fused_attn_fn = library.get_function("fused_attention", None).ok()?;
        let fused_attn_pipeline = device
            .new_compute_pipeline_state_with_function(&fused_attn_fn)
            .ok()?;
        ...
        let silu_pipeline = device
            ...
            .ok()?;
        let gelu_tanh_pipeline = device
            .new_compute_pipeline_state_with_function(&gelu_tanh_fn)
            .ok()?;
        ...
        let layer_norm_pipeline = device
            .new_compute_pipeline_state_with_function(&layer_norm_fn)
            .ok()?;
        let layer_norm_no_bias_pipeline = device
            .new_compute_pipeline_state_with_function(&layer_norm_no_bias_fn)
            .ok()?;

        // V-norm (parameter-free RMSNorm, Gemma 4)
        let v_norm_fn = library.get_function("v_norm", None).ok()?;
        let v_norm_pipeline = device
            .new_compute_pipeline_state_with_function(&v_norm_fn)
            .ok()?;
        let v_norm_batched_fn = library.get_function("v_norm_batched", None).ok()?;
        let v_norm_batched_pipeline = device
            .new_compute_pipeline_state_with_function(&v_norm_batched_fn)
            .ok()?;

        // Scale vector (per-layer scalar multiplier, Gemma 4)
        let scale_vector_fn = library.get_function("scale_vector", None).ok()?;
        let scale_vector_pipeline = device
            .new_compute_pipeline_state_with_function(&scale_vector_fn)
            .ok()?;

        // KV cache attention
        let kv_attend_fn = library.get_function("kv_attention", None).ok()?;
        let kv_append_fn = library.get_function("kv_cache_append", None).ok()?;
        let kv_attend_pipeline = device
            .new_compute_pipeline_state_with_function(&kv_attend_fn)
            .ok()?;
        let kv_append_pipeline = device
            .new_compute_pipeline_state_with_function(&kv_append_fn)
            .ok()?;

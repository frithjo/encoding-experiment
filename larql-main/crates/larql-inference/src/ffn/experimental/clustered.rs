...
impl ClusteredGateIndex {
    ...
    fn kmeans(
        w_gate: &ndarray::ArrayBase<impl ndarray::Data<Elem = f32>, ndarray::Ix2>,
        k: usize,
        iters: usize,
    ) -> LayerClusters {
        ...
        for c in 0..k {
            centroids.row_mut(c).assign(&w_gate.row(c * n / k));
        }
    ...
    pub fn lookup(
        ...
        let lc = match self.layers.get(&layer) {
            Some(lc) => lc,
            None => return vec![],
        };
        ...
        for &(cid, _) in &indexed {
            features.extend_from_slice(&lc.members[cid]);
        }

...
use larql_inference::layer_graph::detect_template_with_cache;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 2 {
        ...
        eprintln!("\nExample:");
        eprintln!(
            "  {} /path/to/gemma3-4b.vindex /path/to/gemma-3-4b",
            args[0]
        );
        std::process::exit(1);
    }
    ...
    println!(
        "Benchmarking {} test prompts with {} templates",
        test_prompts.len(),
        templates.len()
    );
    println!();
    ...
    for prompt in &test_prompts {
        ...
        match detect_template_with_cache(&token_ids, &templates, &vindex) {
            Some((template_id, _cache)) => {
                ...
                println!(
                    "✓ Template detected: {} ({})",
                    templates[template_id].name, prompt
                );
    ...
    println!(
        "  Hit rate: {:.1}%",
        (cache_hits as f64 / test_prompts.len() as f64) * 100.0
    );

    if cache_hits == 0 {
        println!("\nNo cache hits - ensure cached_residuals.bin exists in the vindex");
        println!(
            "Run: larql cache-templates --vindex {} --model <model_path>",
            vindex_path.display()
        );
    }

...
use larql_vindex::{
    load_vindex_config, load_vindex_tokenizer, SilentLoadCallbacks, TokenizerArc, VectorIndex,
};
...
impl PyWalkModel {
    ...
    fn new(path: &str, top_k: usize) -> PyResult<Self> {
        ...
        let (weights, mmaps) =
            load_mmap_weights(dir).map_err(|e| pyo3::exceptions::PyIOError::new_err(e))?;

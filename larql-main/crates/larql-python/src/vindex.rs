...
use larql_vindex::{
    load_vindex_config, load_vindex_embeddings, load_vindex_tokenizer, FeatureMeta,
    SilentLoadCallbacks, TokenizerArc, VectorIndex, VindexConfig, WalkHit,
};
...
impl PyVindex {
    ...
    fn infer(
        ...
        let walk_ffn =
            larql_inference::WalkFfn::new(&infer_state.weights, &self.index, top_k_features);
    ...
    fn infer_trace<'py>(
        ...
        let residuals: Vec<Bound<'py, PyArray1<f32>>> = result
            .residuals
            .into_iter()

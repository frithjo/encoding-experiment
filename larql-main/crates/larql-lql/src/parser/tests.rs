...
fn parse_analyze_infer_minimal_rejected() {
    ...
    assert!(
        err.contains("MODE clause"),
        "error should mention MODE clause requirement"
    );
}
...
fn parse_analyze_infer_workflow_probe() {
    let stmt =
        parse(r#"ANALYZE INFER "Test prompt" MODE WORKFLOW_PROBE FALSE_SPANS ("hallucination");"#)
            .unwrap();
    match stmt {
        Statement::AnalyzeInfer {
            mode,
...
fn parse_analyze_infer_fact_probe_truth_only() {
    let stmt =
        parse(r#"ANALYZE INFER "Test prompt" MODE FACT_PROBE TRUTH_SPANS ("Paris");"#).unwrap();
    match stmt {
        Statement::AnalyzeInfer {
            mode,
            truth_spans,
...
fn parse_analyze_infer_fact_probe_false_only() {
    let stmt =
        parse(r#"ANALYZE INFER "Test prompt" MODE FACT_PROBE FALSE_SPANS ("London");"#).unwrap();
    match stmt {
        Statement::AnalyzeInfer {
            mode,
            truth_spans,
...
fn parse_analyze_infer_fact_probe_no_spans_rejected() {
    ...
    let err = result.unwrap_err().to_string();
    assert!(
        err.contains("TRUTH_SPANS") || err.contains("FALSE_SPANS"),
        "error should mention span requirement"
    );
}
...
fn parse_analyze_infer_workflow_probe_no_spans_rejected() {
    ...
    let err = result.unwrap_err().to_string();
    assert!(
        err.contains("FALSE_SPANS"),
        "error should mention FALSE_SPANS requirement"
    );
}
...
fn parse_select_tokens() {
    let stmt = parse("SELECT * FROM TOKENS LIMIT 5;").unwrap();
    match stmt {
        Statement::Select {
            source,
            conditions,
            ...
            assert!(matches!(source, SelectSource::Tokens));
            assert!(conditions.is_empty());
            assert_eq!(limit, Some(5));
        }
        _ => panic!("expected Select"),
...
#[test]
fn parse_select_tokens_with_filters() {
    let stmt =
        parse(r#"SELECT * FROM TOKENS WHERE shape = "upper" AND band = "knowledge" LIMIT 10;"#)
            .unwrap();
    match stmt {
        Statement::Select {
            source,
...
#[test]
fn parse_show_tokens_minimal() {
    let stmt = parse("SHOW TOKENS;").unwrap();
    match stmt {
        ...
        _ => panic!("expected ShowTokens"),
    }
}

#[test]
fn parse_show_tokens_at_layer() {
    let stmt = parse("SHOW TOKENS AT LAYER 14;").unwrap();
    match stmt {
        Statement::ShowTokens { layer, .. } => assert_eq!(layer, Some(14)),
        _ => panic!("expected ShowTokens"),
    }
}

#[test]
fn parse_show_tokens_with_where_and_verbose() {
    let stmt =
        parse(r#"SHOW TOKENS WHERE shape = "upper" AND band = "knowledge" VERBOSE;"#).unwrap();
    match stmt {
        Statement::ShowTokens {
            conditions,
            verbose,
...
#[test]
fn parse_show_tokens_group_by_band() {
    let stmt = parse("SHOW TOKENS BY BAND;").unwrap();
    match stmt {
        Statement::ShowTokens { group_by, .. } => {
            assert_eq!(group_by, Some(TokenGroupBy::Band));
        }
        _ => panic!("expected ShowTokens"),
    }
}
...
#[test]
fn parse_show_tokens_group_by_layer_verbose() {
    let stmt = parse("SHOW TOKENS BY LAYER VERBOSE;").unwrap();
    match stmt {
        Statement::ShowTokens {
            group_by, verbose, ..
        } => {
            assert_eq!(group_by, Some(TokenGroupBy::Layer));
            assert!(verbose);
        }
        _ => panic!("expected ShowTokens"),
    }

...
fn parse_select_tokens() {
    ...
    match stmt {
        Statement::Select { source, conditions, limit, .. } => {
...
fn parse_select_tokens_with_filters() {
    let stmt = parse(
        r#"SELECT * FROM TOKENS WHERE shape = "upper" AND band = "knowledge" LIMIT 10;"#,
    ).unwrap();
    match stmt {
        Statement::Select { source, conditions, limit, .. } => {
            assert!(matches!(source, SelectSource::Tokens));
...
fn parse_show_tokens_minimal() {
    ...
    match stmt {
        Statement::ShowTokens { layer, conditions, verbose, group_by, .. } => {
            assert!(layer.is_none());
            assert!(conditions.is_empty());
            assert!(!verbose);
...
fn parse_show_tokens_with_where_and_verbose() {
    let stmt = parse(r#"SHOW TOKENS WHERE shape = "upper" AND band = "knowledge" VERBOSE;"#).unwrap();
    match stmt {
        Statement::ShowTokens { conditions, verbose, group_by, .. } => {
            assert_eq!(conditions.len(), 2);
            assert!(verbose);
            assert!(group_by.is_none());
        }
...
fn parse_show_tokens_group_by_layer_verbose() {
    ...
    match stmt {
        Statement::ShowTokens { group_by, verbose, .. } => {

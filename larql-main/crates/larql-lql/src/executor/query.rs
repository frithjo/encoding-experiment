...
use super::helpers::{
    classify_token_shape, entity_token_kind, is_content_token, token_shape_name, TokenShape,
};
use super::Session;
use crate::ast::*;
...
impl Session {
    ...
    pub(crate) fn exec_select_entities(
        ...
        for layer in &scan_layers {
            ...
            for feat in 0..nf {
                if let Some(meta) = patched.feature_meta(*layer, feat) {
                    ...
                    let Some(kind) = entity_token_kind(&tok) else {
                        continue;
                    };
        ...
        let mut entities: Vec<(String, EntityInfo)> = entity_counts.into_iter().collect();
...
pub(crate) fn describe_resolve_bands(

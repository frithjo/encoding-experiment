...
use super::Session;
use super::helpers::{classify_token_shape, entity_token_kind, is_content_token, token_shape_name, TokenShape};
...
impl Session {
    ...
    pub(crate) fn exec_describe(
        ...
        if edges.is_empty() {
            ...
            let walk_layer_range = describe_scan_layers_to_range(&scan_layers);
            match self.exec_explain(entity, walk_layer_range.as_ref(), false) {
                Ok(walk_lines) => {
                    out.push(String::new());
                ...
                Err(e) => {
                    out.push(String::new());
                    out.push(format!("  EXPLAIN WALK — skipped ({e})"));
                }
            }

            match self.exec_infer_trace(entity, Some(5), band, false, false) {
                Ok(infer_lines) => {
                    out.push(String::new());
                ...
                Err(e) => {
                    out.push(String::new());
                    out.push(format!("  EXPLAIN INFER — skipped ({e})"));
                }
            }
    ...
    pub(crate) fn exec_select_tokens(
        &self,
        conditions: &[Condition],
        limit: Option<u32>,
    ) -> Result<Vec<String>, LqlError> {
        ...
        let shape_filter = conditions.iter().find(|c| c.field == "shape").and_then(|c| {
            if let Value::String(ref s) = c.value { Some(s.to_lowercase()) } else { None }
        });
        let type_filter = conditions.iter().find(|c| c.field == "type" || c.field == "kind").and_then(|c| {
            if let Value::String(ref s) = c.value { Some(s.to_lowercase()) } else { None }
        });
        let band_filter = conditions.iter().find(|c| c.field == "band").and_then(|c| {
            if let Value::String(ref s) = c.value { Some(s.to_lowercase()) } else { None }
        });
        ...
        let mut token_counts: HashMap<String, TokenInfo> = HashMap::new();
        ...
        for layer in &scan_layers {
            ...
            for feat in 0..nf {
                if let Some(meta) = patched.feature_meta(*layer, feat) {
                    ...
                    let entity_kind = entity_token_kind(&tok);
                    let shape_name = token_shape_name(shape);
                    let band_name = if *layer >= bands.knowledge.0 && *layer <= bands.knowledge.1 {
                        "knowledge"
                    ...
                    if let Some(sf) = &shape_filter {
                        if shape_name != sf {
                            continue;
                        }
                    }
                    if let Some(tf) = &type_filter {
                        let kind = entity_kind.unwrap_or("none");
                        if kind != tf {
                            continue;
                        }
                    }
                    if let Some(bf) = &band_filter {
                        if band_name != bf {
                            continue;
                        }
                    }

                    let entry = token_counts.entry(tok).or_insert(TokenInfo {
                        shape,
                        entity_kind,
                        count: 0,
                        max_score: 0.0,
        ...
        let mut tokens: Vec<(String, TokenInfo)> = token_counts.into_iter().collect();
        tokens.sort_by(|a, b| {
            b.1.count
                .cmp(&a.1.count)
                .then_with(|| {
        ...
        for (tok, info) in &tokens {
            let bands_seen = format!(
                "{}{}{}",
                if info.syntax_hits > 0 { "S" } else { "-" },
                if info.knowledge_hits > 0 { "K" } else { "-" },
                if info.output_hits > 0 { "O" } else { "-" },
            );
            out.push(format!(
                "{:<24} {:<12} {:<10} {:<8} {:>10} {:>10.4}",
                if tok.is_empty() { "∅" } else { tok.as_str() },
                token_shape_name(info.shape),
                info.entity_kind.unwrap_or("none"),
                bands_seen,
                info.count,
                info.max_score
            ));
        ...
        if tokens.is_empty() {
            out.push("  (no matching tokens)".into());
        }
    ...
    pub(crate) fn exec_select_entities(
        ...
        let mut entity_counts: std::collections::HashMap<String, EntityInfo> =
        ...
        for layer in &scan_layers {
            ...
            for feat in 0..nf {
                if let Some(meta) = patched.feature_meta(*layer, feat) {
                    ...
                    let Some(kind) = entity_token_kind(&tok) else { continue; };
                    if entity_filter.is_none() && kind == "acronym" {
                        continue;
                    }
                    ...
                    let (band_weight, syntax_hit, knowledge_hit, output_hit) =
                        if *layer >= bands.knowledge.0 && *layer <= bands.knowledge.1 {
                            (1.0, 0usize, 1usize, 0usize)
                        } else if *layer >= bands.output.0 && *layer <= bands.output.1 {
                            (0.7, 0usize, 0usize, 1usize)
                        } else {
                            (0.35, 1usize, 0usize, 0usize)
                        };
                    let entry = entity_counts.entry(tok).or_insert(EntityInfo {
                        count: 0,
                        max_score: 0.0,
                        weighted_score: 0.0,
                        kind,
                    ...
                    entry.count += 1;
                    entry.weighted_score += band_weight;
                    entry.syntax_hits += syntax_hit;
                    entry.knowledge_hits += knowledge_hit;
                    entry.output_hits += output_hit;
                    if meta.c_score > entry.max_score { entry.max_score = meta.c_score; }
                }
            }
        }

        let mut entities: Vec<(String, EntityInfo)> = entity_counts
            .into_iter()
            .collect();
        ...
        out.push(format!(
            "{:<24} {:<10} {:<8} {:>10} {:>10}",
            "Entity", "Type", "Bands", "Features", "Max Score"
        ));
        out.push("-".repeat(70));

        for (tok, info) in &entities {
            let bands_seen = format!(
                "{}{}{}",
                if info.syntax_hits > 0 { "S" } else { "-" },
                if info.knowledge_hits > 0 { "K" } else { "-" },
            ...
            out.push(format!(
                "{:<24} {:<10} {:<8} {:>10} {:>10.4}",
                tok, info.kind, bands_seen, info.count, info.max_score
            ));
        }
...
pub(crate) fn describe_resolve_bands(config: &larql_vindex::VindexConfig) -> larql_vindex::LayerBands {
    let last = config.num_layers.saturating_sub(1);
    config
        .layer_bands
        .clone()
...
/// Min/max layer span for `scan_layers`, for forwarding the same scope to `EXPLAIN WALK`.
fn describe_scan_layers_to_range(scan_layers: &[usize]) -> Option<Range> {
    if scan_layers.is_empty() {
        return None;
    }
    let start = *scan_layers.iter().min()?;
    let end = *scan_layers.iter().max()?;
    Some(Range {
        start: start as u32,
        end: end as u32,
    })
...
/// Per-target accumulator for the walk-collected edges (shared with larql-vindex).
type DescribeEdge = larql_vindex::CollectedDescribeEdge;

...
mod tests {
    ...
    fn put_and_get() {
        let cache = DescribeCache::new(60);
        let key = DescribeCache::key("model", "France", "knowledge", 20, 5.0, "-", false);
    ...
    #[test]
    fn key_format() {
        let key = DescribeCache::key("gemma-3-4b-it", "France", "knowledge", 20, 5.0, "-", false);
        assert_eq!(key, "gemma-3-4b-it:France:knowledge:20:5:-:0");
    }

    #[test]
    fn different_params_different_keys() {
        let k1 = DescribeCache::key("model", "France", "knowledge", 20, 5.0, "-", false);
        let k2 = DescribeCache::key("model", "Germany", "knowledge", 20, 5.0, "-", false);
        let k3 = DescribeCache::key("model", "France", "syntax", 20, 5.0, "-", false);
        assert_ne!(k1, k2);
        assert_ne!(k1, k3);
    }
}

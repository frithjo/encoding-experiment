...
pub fn run_all_strategies(
    ...
    let markov_preds =
        logits_to_predictions_pub(bench.weights, &markov.hidden, bench.tokenizer, top_k, 1.0);
    let markov_us = t0.elapsed().as_secs_f64() * 1e6;

    let markov_top1 = markov_preds
        .predictions
        .first()
        .map(|(t, _)| t.clone())
        .unwrap_or_default();

    let (_markov_mse, markov_cosine) =
        markov_layer::compare_hidden_states(&kv.hidden, &markov.hidden);

    results.push(RealModelResult {
        strategy: "Markov Residual Stream".to_string(),
        prompt: prompt.to_string(),
        top1_token: markov_top1.clone(),
        top1_prob: markov_preds
            .predictions
            .first()
            .map(|(_, p)| *p)
            .unwrap_or(0.0),
...
pub fn format_results(results: &[RealModelResult]) -> String {
    ...
    if let Some(cosine) = results
        .iter()
        .find(|r| r.strategy.contains("Markov"))
        .and_then(|r| r.hidden_cosine)
    {
        out.push_str(&format!(

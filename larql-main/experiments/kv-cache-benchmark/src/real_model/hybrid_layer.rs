...
use larql_inference::forward::{apply_norm, embed_tokens_pub, run_ffn};
...
pub fn capture_per_head_attention(weights: &ModelWeights, token_ids: &[u32]) -> Vec<PerHeadOutput> {
    ...
    for layer in 0..num_layers {
        ...
        let (h_post_attn, attn_projected, _attn_weights) =
            run_attention_block(weights, &h, layer, false).expect("attention failed");
...
pub fn run_hybrid_inference(
    ...
    for layer in 0..num_layers {
        let layer_results: Vec<&HeadClassResult> = classification
            .results
            .iter()
...
pub fn format_classification(cls: &ModelHeadClassification) -> String {
    ...
    if num_q > 0 {
        ...
        for layer in 0..num_layers {
            let layer_results: Vec<&HeadClassResult> =
                cls.results.iter().filter(|r| r.layer == layer).collect();
            let static_count = layer_results.iter().filter(|r| r.is_static).count();
            let dynamic_count = num_q - static_count;
            let mean_cos: f32 =
                layer_results.iter().map(|r| r.mean_cosine).sum::<f32>() / num_q as f32;

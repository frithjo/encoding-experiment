...
impl HybridCrackedAttention {
    ...
    pub fn new(static_head_fraction: f64, dynamic_layers: usize, dynamic_window: usize) -> Self {
        Self {
...
mod tests {
    ...
    fn test_hybrid_template_cache_shared() {
        ...
        assert!(
            shared < 5_000_000,
            "Shared infra should be <5MB per template"
        );
    }
    ...
    fn test_hybrid_compression_ratio() {
        ...
        assert!(
            ratio_370k > 10.0,
            "370K compression {ratio_370k:.1}x too low"
        );
    }
}

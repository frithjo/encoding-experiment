...
fn test_hybrid_head_classification_real() {
    ...
    println!(
        "{}",
        kv_cache_benchmark::real_model::hybrid_layer::format_classification(&classification)
    );
    ...
    println!(
        "Dynamic heads: {}/{}",
        classification.dynamic_count, classification.total_heads
    );
...
fn test_hybrid_inference_vs_baseline() {
    ...
    println!(
        "{:<45} {:>10} {:>12} {:>12} {:>8}",
        "Prompt", "Top-1", "Dyn KV", "Full KV", "Ratio"
    );
    println!("{}", "-".repeat(90));
...
fn test_hybrid_head_classification_incontext() {
    ...
    println!(
        "{:>5} {:>12} {:>12} {:>10}",
        "Layer", "Param cos", "InCtx cos", "Diff"
    );
    println!("{}", "-".repeat(45));
    ...
    for layer in 0..num_layers {
        ...
        for head in 0..num_q {
            ...
            inctx_cosines.push(if pairs > 0 {
                (cos_sum / pairs as f64) as f32
            } else {
                0.0
            });
        }
        ...
        if param_all_static == inctx_all_static {
            both_agree += 1;
        }

        let diff = inctx_mean - param_mean;
        let marker = if !param_all_static || !inctx_all_static {
            " ←"
        } else {
            ""
        };
        println!(
            "L{:<4} {:>12.4} {:>12.4} {:>+10.4}{marker}",
            layer, param_mean, inctx_mean, diff
        );
    }
